...
<body class="p-4">
    <div class="max-w-3xl mx-auto overflow-hidden bg-white rounded-lg custom-shadow">
                ...
                <div class="flex h-full">
                    ...
                    <div class="flex-1">
                        ...
                        <div class="p-1 ">
                            ...
                            <p class="pl-2 text-lg font-bold pukimak">Ktiq</p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="my-slider">
                    @foreach ($records as $record)
                        <div class="p-2 mx-3 text-center text-white rounded bg-mtq-blue">
                            <h3 class="mb-1 text-xs">Bobot Materi</h3>
                            <p class="text-3xl font-bold">{{ $currentRecord->bobot_materi }}</p>
                        </div>
                        <div class="p-2 mx-3 text-center text-white rounded bg-mtq-blue">
                            <h3 class="mb-1 text-xs">Kaidah Dan Gaya Bahasa</h3>
                            <p class="text-3xl font-bold">{{ $currentRecord->kaidah_dan_gaya_bahasa }}</p>
                        </div>
                        <div class="p-2 mx-3 text-center text-white rounded bg-mtq-blue">
                            <h3 class="mb-1 text-xs">logika dan organisasi pesan</h3>
                            <p class="text-3xl font-bold">{{ $currentRecord->logika_dan_organisasi_pesan }}</p>
                        </div>
                        <div class="p-2 mx-3 text-center text-white rounded bg-mtq-blue">
                            <h3 class="mb-1 text-xs">presentasi</h3>
                            <p class="text-3xl font-bold">{{ $currentRecord->presentasi }}</p>
                        </div>
                    @endforeach
                </div>
            </div>
        ...
        <div class="flex items-end justify-between px-4 pb-4">
            @if ($previousRecord)
                <a href="{{ route('nilai-ktiq.index', $previousRecord->id) }}" class="px-6 py-2 text-sm font-semibold text-white bg-gray-800 rounded">Back</a>
            @else
                <button class="px-6 py-2 text-sm font-semibold text-white bg-gray-400 rounded" disabled>Back</button>
            @endif

            @if ($nextRecord)
                <a href="{{ route('nilai-ktiq.index', $nextRecord->id) }}" class="px-6 py-2 text-sm font-semibold text-white bg-gray-800 rounded">Next</a>
            @else
