...
<body class="p-4">
    <div class="max-w-3xl mx-auto overflow-hidden bg-white rounded-lg custom-shadow">
                ...
                <div class="flex h-full">
                    ...
                    <div class="flex-1">
                        ...
                        <div class="p-1 ">
                            ...
                            <p class="pl-2 text-lg font-bold pukimak">Naskah</p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="my-slider">
                    @foreach ($records as $record)
                        <div class="p-2 mx-3 text-center text-white rounded bg-mtq-blue">
                            <h3 class="mb-1 text-xs">Kebenaran Kaidah Kaht Wajib </h3>
                            <p class="text-3xl font-bold">{{ $currentRecord->kebenaran_kaidah_khat_wajib }}</p>
                        </div>
                        <div class="p-2 mx-3 text-center text-white rounded bg-mtq-blue">
                            <h3 class="mb-1 text-xs">Keindahan Khat Wajib</h3>
                            <p class="text-3xl font-bold">{{ $currentRecord->keindahan_khat_wajib }}</p>
                        </div>
                        <div class="p-2 mx-3 text-center text-white rounded bg-mtq-blue">
                            <h3 class="mb-1 text-xs">Kebenaran Khaidah Khat Pilihan</h3>
                            <p class="text-3xl font-bold">{{ $currentRecord->kebenaran_kaidah_khat_pilihan }}</p>
                        </div>
                        <div class="p-2 mx-3 text-center text-white rounded bg-mtq-blue">
                            <h3 class="mb-1 text-xs">keindahan khat pilihan</h3>
                            <p class="text-3xl font-bold">{{ $currentRecord->keindahan_khat_pilihan}}</p>
                        </div>
                    @endforeach
                </div>
            </div>
        ...
        <div class="flex items-end justify-between px-4 pb-4">
            @if ($previousRecord)
                <a href="{{ route('nilai-anak.index', $previousRecord->id) }}" class="px-6 py-2 text-sm font-semibold text-white bg-gray-800 rounded">Back</a>
            @else
                <button class="px-6 py-2 text-sm font-semibold text-white bg-gray-400 rounded" disabled>Back</button>
            @endif

            @if ($nextRecord)
                <a href="{{ route('nilai-anak.index', $nextRecord->id) }}" class="px-6 py-2 text-sm font-semibold text-white bg-gray-800 rounded">Next</a>
            @else
